  ...
  <div class="electrical-page">
    <div class="page-layout" :class="{ 'detail-open': isDetailPanelOpen }">
      ...
      <div class="main-content">
          ...
          <div class="filter-group">
            <div class="filter-item">
              ...
              <select id="unit" v-model="selectedUnit" class="form-select">
                ...
                <option
                  v-for="unit in electricalStore.unitSystems"
            ...
            <div class="filter-item">
              ...
              >
                <option value="">{{ t("common.select") }}</option>
                <option
                  v-for="category in electricalStore.secondDepth"
            ...
            <div class="filter-item">
              ...
              <input
                type="text"
                id="search"
                :placeholder="t('placeholder.electricalSearch')"
          ...
          <div class="detail-search-header">
            ...
            <div class="detail-search-row">
              <div class="detail-search-item">
                <label>{{ t("electrical.electricalSubCategory") }}</label>
                <select
              ...
              <div class="detail-search-item">
                <label>{{ t("electrical.electricalCategory") }}</label>
                <select
        ...
        <!-- 전기자재 리스트 헤더 -->
        <div class="electrical-list-header">
          <h2>{{ t("common.electricalList") }}</h2>
        ...
        <div class="pagination-container">
          <div class="total-count">
            {{
              t("common.totalCount", {
                count: electricalStore.searchResults?.total || 0,
              })
            }}
          </div>
          <Pagination
    ...
    <div v-if="isRegistModalOpen" class="modal-overlay">
      <div class="modal-container" style="max-width: 1600px; width: 98%">
        ...
        <div class="modal-body">
          <!-- 등록 폼 -->
          <div class="electrical-register-form">
            <!-- 상단 검색/필터 영역 -->
            <div class="filter-bar" style="display: flex; gap: 10px; align-items: center; flex-wrap: nowrap; overflow: hidden;">
              ...
              <div class="group-form inline" style="flex-shrink: 0; gap: 4px;">
                ...
                <select
                  class="input select-md"
                  v-model="registerSelectedElectricalName"
                  :disabled="registerIsRegistered"
                  style="max-width: 80px; width: 80px; flex-shrink: 0;"
                >
                  <option value="">{{ t("common.select") }}</option>
                  <option
                    v-for="electrical in electricalStore.secondDepth"
              ...
              <div class="group-form inline right-align" style="flex-shrink: 0; gap: 6px;">
                ...
                <button
                  type="button"
                  class="btn-register"
                  @click="handleElectricalRegister"
                >
                  {{ t("common.electricalRegistration") }}
                </button>
              </div>
            </div>
...
const tableColumns: TableColumn[] = [
  { key: "no", title: t("columns.electrical.no"), width: "60px", sortable: false },
  {
  ...
  {
    key: "middle_category",
    title: t("columns.electrical.middleCategory"),
    width: "120px",
    sortable: false,
  },
  {
    key: "electrical_type",
    title: t("columns.electrical.electricalType"),
    width: "120px",
    sortable: false,
  },
  {
    key: "equipment_code",
    title: t("columns.electrical.pcId"),
    width: "180px",
    sortable: false,
  },
  {
    key: "vendor_name",
    title: t("columns.electrical.company"),
    width: "150px",
    sortable: false,
  },
  {
    key: "model_number",
    title: t("columns.electrical.model"),
    width: "150px",
    sortable: false,
  },
  {
    key: "unit_price",
    title: t("columns.electrical.unitPrice"),
    width: "120px",
    sortable: false,
  },
  {
    key: "details",
    title: t("columns.electrical.details"),
    width: "100px",
    sortable: false,
  },
];
...
const registerSelectedElectricalName = ref("");
...
const parseHierarchy = (fullHierarchy?: string) => {
  ...
  return {
    ...
    electricalType: levels.length > 3 ? parseLevel(levels[3]) : null, // 레벨4
  };
};
...
const getElectricalType = (item: MachineItem): string => {
  const hierarchyInfo = item.hierarchy_info as { full_hierarchy?: string } | undefined;
  if (hierarchyInfo?.full_hierarchy) {
    const parsed = parseHierarchy(hierarchyInfo.full_hierarchy);
    if (parsed.electricalType) return parsed.electricalType;
  }
...
const specVerticalData = computed(() => {
  ...
  // 1. 고정 필드
  data.push({
    columnName: t("columns.electrical.type"),
    value: item.equipment_type_name || "-",
  });
  data.push({
    columnName: t("columns.electrical.code"),
  ...
  data.push({
    columnName: t("columns.electrical.company"),
...
const totalPagesComputed = computed(
  () => electricalStore.searchResults?.total_pages || 1
);
...
const validateRegisterBasicSelections = (): boolean => {
  ...
  if (!registerSelectedElectricalName.value) {
...
const onDownloadExcelTemplate = async () => {
  ...
  try {
    const response = await electricalStore.downloadExcelTemplate(
      registerSelectedElectricalName.value
    );
...
// 배관 등록 핸들러
const handleElectricalRegister = async () => {
  if (!validateRegisterBasicSelections()) return;
  ...
  try {
    // electricalStore의 uploadElectricalExcel 함수 호출
    const response = await electricalStore.uploadElectricalExcel(
      registerSelectedElectricalName.value,
...
const handleModelRegister = async () => {
  ...
  try {
    // electricalStore의 uploadModelZip 함수 호출
    const response = await electricalStore.uploadModelZip(
      registerSelectedElectricalName.value,
  ...
  if (
    ...
    try {
      ...
      for (const item of selectedItems.value) {
        ...
        await electricalStore.deleteElectrical(item.equipment_id, deleteParams);
      }
...
const openDetailPanel = async (item: MachineItem) => {
  ...
  try {
    ...
    if (item.root_equipment_type) {
      const response = await electricalStore.fetchElectricalDetailCommon(
        ...
        for (const [key, originalField] of Object.entries(
          ...
          if (isChanged) {
            ...
            try {
              ...
              await electricalStore.createPriceHistory({
...
const handleFieldChange = (fieldName: string, value: string | boolean | number) => {
  ...
  // editData에 반영
  // 배관코드 필드 확인 (columns.electrical.code)
  if (fieldName === t("columns.electrical.code")) {
    // 60바이트 제한 적용
  ...
  // 제조사 필드 확인 (columns.electrical.company)
  else if (fieldName === t("columns.electrical.company")) {
  ...
  // 모델명 필드 확인 (columns.electrical.model)
  else if (fieldName === t("columns.electrical.model")) {
...
const loadData = async () => {
  try {
    ...
    if (
      ...
      alert(t("messages.warning.selectElectricalType"));
      return;
    }
    ...
    if (electricalStore.searchResults?.items) {
      machineList.value = electricalStore.searchResults
...
const handleMachineCategoryChange = async () => {
  if (selectedMachineCategory.value) {
    await electricalStore.fetchThirdDepth(selectedMachineCategory.value, 3);
  }
  ...
  try {
    if (selectedMachineCategory.value) {
      const selected = electricalStore.secondDepth.find(
        (row: any) => row.code_key === selectedMachineCategory.value
      );
      if (selected?.code_group) {
        const res = await electricalStore.fetchDepthDetail(selected.code_group, 3);
...
const handleHeaderMachineCategoryChange = async () => {
  ...
  try {
    const res = await electricalStore.fetchDepthDetailBySearchType(searchKey);
...
const handleHeaderMachineSubCategoryChange = async () => {
  ...
  if (!searchKey) return;
  try {
    const res = await electricalStore.fetchThirdDepth(searchKey, 4);
    ...
    if (Array.isArray(codes) && codes.length > 0) {
      ...
      const searchRes = await electricalStore.fetchDepthDetailBySearchType(
        searchKey
      );
...
onMounted(async () => {
  await electricalStore.fetchCommonCodes("ELCTR");
  await loadData();
});
</script>
