...
export const useVendorsStore = defineStore("vendors", () => {
  ...
  const fetchVendorDetail = async (vendorId: string) => {
    ...
    try {
      ...
      const response = await request(`/api/vendor/detail/${vendorId}`, undefined, {
        method: "GET",
      });
  ...
  const updateVendor = async (vendorId: string, vendorData: VendorUpdateRequest) => {
    ...
    try {
      ...
      if (vendorData.certification !== undefined) requestData.certificaton = vendorData.certification || "";  // 백엔드 API 필드명 (오타)
      if (vendorData.note !== undefined) requestData.note = vendorData.note || "";
      if (vendorData.isPreferred !== undefined) requestData.is_preferred = vendorData.isPreferred ?? false;
      if (vendorData.preferredLevel !== undefined) requestData.preferred_level = vendorData.preferredLevel || null;
      if (vendorData.isActive !== undefined) requestData.is_active = vendorData.isActive ?? true;

      console.log("updateVendor 호출 - vendorId:", vendorId, "requestData:", requestData);

      const response = await request(`/api/vendor/update/${vendorId}`, undefined, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const deleteVendor = async (vendorId: string) => {
    ...
    try {
      ...
      const response = await request(`/api/vendor/delete/${vendorId}`, undefined, {
        method: "DELETE",
      });
  ...
  const loadVendorData = async () => {
    ...
    try {
      ...
      error.value =
        err instanceof Error ? err.message : "공급업체 데이터 로드에 실패했습니다.";
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  const changePage = async (page: number) => {
    console.log("changePage 호출 - 이전 페이지:", currentPage.value, "→ 새 페이지:", page);
