      ...
      <div class="tab-content">
       <div v-if="activeTab >= 0" class="content">
        ...
        <div class="action-bar">
         ...
         <div class="btns">
          <button class="btn btn-add">계산식 추가</button> 
         </div>
        </div>
        
        <DataTable :columns="gridColumns2" :data="currentGridData2">
          <template #cell-formula="{ item, index }">
            <input v-if="index === currentGridData2.length - 1" type="file" class="form-input" />
          ...
          <template #cell-remarks="{ item, index }">
            <input v-if="index === currentGridData2.length - 1" type="text" v-model="item.remarks" class="form-input" />
...
interface GridRow2 {
 ...
 const gridData = ref<GridRow[]>([
   ...
   { id: 3, item: "SS", influent: 120.0, unit: "mg/L", display: "N", remarks: "측정 불가" },
   { id: 4, item: "T-N", influent: 40.0, unit: "mg/L", display: "Y", remarks: "" },
   { id: 5, item: "T-P", influent: 4.0, unit: "mg/L", display: "Y", remarks: "" },
 ]);
