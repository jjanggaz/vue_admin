...
export const useInflowStore = defineStore("inflow", {
  ...
  actions: {
    ...
    // 유입종류별 파라미터 조회
    async fetchWaterFlowTypeParameters(flowDirection: string, flowTypeCode: string) {
      this.loading = true;
      this.error = null;
    ...
    async fetchWaterQualityParameters() {
      ...
      try {
        const response = await request("/api/inflow/water-quality-parameters", undefined, {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        if (response.response && response.response.items) {
          ...
          const filteredItems = response.response.items.filter((item: any) => 
    ...
    async fetchCommonCodes(codeGroup?: string, parentKey?: string, isActive?: boolean) {
      ...
      try {
        ...
        const url = `/api/inflow/common/codes${queryString ? `?${queryString}` : ''}`;
    ...
    // 유입종류 파라미터 등록
    async createWaterFlowTypeParameters(parameterData: WaterFlowTypeParameterCreateRequest) {
      this.loading = true;
      this.error = null;
      ...
      try {
        ...
        const response = await request("/api/inflow/parameters/create", undefined, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
    ...
    // 유입종류 수정
    async updateWaterFlowType(flowTypeId: string, waterFlowTypeData: Partial<WaterFlowTypeFormData>) {
      this.loading = true;
      this.error = null;
